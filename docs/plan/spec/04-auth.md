...
Conventions: "main" = the main actor (the app target's default isolation, `SWIFT_DEFAULT_ACTOR_ISOLATION = MainActor` `[tooling §3.3]`); "the item" = the Keychain generic-password item `service = "de.newtelco.minimail"`, `account = "oauth.authState"`; "the provider" = `AppAuthTokenProvider`; "the store" = `AuthStore`. Line references `architecture.md §5.3` refer to the design document as committed.
...
| `kSecAttrService` | `de.newtelco.minimail` |
...
https://accounts.google.com/o/oauth2/v2/auth?
  ...
  &hd=newtelco.de                                    (attempt 1 only; dropped by the optional-parameter retry)
...
|---|---|
| title | `minimail` |
| subtitle | `Gmail for newtelco.de` |
...
VStack(spacing: 16)                                        .padding(.horizontal, 32) .frame(maxWidth: .infinity, maxHeight: .infinity) .background(themeTokens.background.ignoresSafeArea())
 ...
 ├─ Text("Gmail for newtelco.de")                          .font(.subheadline) .foregroundStyle(themeTokens.secondaryText)
 ├─ Spacer()
...
|---|---|---|---|
| `minimailTests/Auth/KeychainTests.swift` | `testServiceConstant` | — | `Keychain.service == "de.newtelco.minimail"` |
...
| | `testEndpointsAndScope` | any config | `authorizationEndpoint == "https://accounts.google.com/o/oauth2/v2/auth"`, `tokenEndpoint == "https://oauth2.googleapis.com/token"`, `revocationEndpoint == "https://oauth2.googleapis.com/revoke"`, `scopes == ["https://www.googleapis.com/auth/gmail.modify"]`, `hostedDomain == "newtelco.de"` |
...
9. Manual device step (real OAuth client, owner's phone, `Config/Google.xcconfig` filled locally): tap "Sign in with Google" → Google sheet with the account chooser (shared session) → consent → the app shows `SignedInPlaceholderView` with "Signed in as <owner e-mail>" only after module 05 wires `fetchProfileEmail`; before that, "Loading your inbox…". Kill and relaunch → still signed in without a sheet (Keychain + `load()`); `log stream --predicate 'subsystem == "de.newtelco.minimail" AND category == "auth"'` shows `routing: keychain=true`, `load: authorized=true`.
